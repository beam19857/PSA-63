...
// QueryPositionUser chains the current query on the PositionUser edge.
func (pq *PositionQuery) QueryPositionUser() *UserQuery {
...
//  WithPositionUser tells the query-builder to eager-loads the nodes that are connected to
// the "PositionUser" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PositionQuery) WithPositionUser(opts ...func(*UserQuery)) *PositionQuery {
	query := &UserQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withPositionUser = query
	return pq
}
...
func (pq *PositionQuery) sqlAll(ctx context.Context) ([]*Position, error) {
	...
	if query := pq.withPositionUser; query != nil {
		...
		query.Where(predicate.User(func(s *sql.Selector) {
			s.Where(sql.InValues(position.PositionUserColumn, fks...))
		}))

...
// The ExpertiseQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type ExpertiseQueryRuleFunc func(context.Context, *ent.ExpertiseQuery) error

// EvalQuery return f(ctx, q).
func (f ExpertiseQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.ExpertiseQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.ExpertiseQuery", q)
}

// The ExpertiseMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type ExpertiseMutationRuleFunc func(context.Context, *ent.ExpertiseMutation) error

// EvalMutation calls f(ctx, m).
func (f ExpertiseMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.ExpertiseMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.ExpertiseMutation", m)
}

...
type UserQuery struct {
	...
	// eager-loading edges.
	withUserDepartment *DepartmentQuery
	withUserExpertise  *ExpertiseQuery
	withUserPosition   *PositionQuery
	withFKs            bool
...
// QueryUserDepartment chains the current query on the UserDepartment edge.
func (uq *UserQuery) QueryUserDepartment() *DepartmentQuery {
...
// QueryUserExpertise chains the current query on the UserExpertise edge.
func (uq *UserQuery) QueryUserExpertise() *ExpertiseQuery {
	query := &ExpertiseQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryUserPosition chains the current query on the UserPosition edge.
func (uq *UserQuery) QueryUserPosition() *PositionQuery {
...
//  WithUserDepartment tells the query-builder to eager-loads the nodes that are connected to
// the "UserDepartment" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithUserDepartment(opts ...func(*DepartmentQuery)) *UserQuery {
	query := &DepartmentQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserDepartment = query
	return uq
}

//  WithUserExpertise tells the query-builder to eager-loads the nodes that are connected to
// the "UserExpertise" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithUserExpertise(opts ...func(*ExpertiseQuery)) *UserQuery {
	query := &ExpertiseQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserExpertise = query
	return uq
}

//  WithUserPosition tells the query-builder to eager-loads the nodes that are connected to
// the "UserPosition" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithUserPosition(opts ...func(*PositionQuery)) *UserQuery {
	query := &PositionQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserPosition = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		_spec       = uq.querySpec()
		loadedTypes = [3]bool{
			uq.withUserDepartment != nil,
			uq.withUserExpertise != nil,
			uq.withUserPosition != nil,
		}
	)
	if uq.withUserDepartment != nil || uq.withUserExpertise != nil || uq.withUserPosition != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := uq.withUserDepartment; query != nil {
	...
	if query := uq.withUserExpertise; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*User)
		for i := range nodes {
		...
		query.Where(expertise.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
	...
	if query := uq.withUserPosition; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*User)
		for i := range nodes {

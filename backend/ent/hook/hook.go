...
// The ExpertiseFunc type is an adapter to allow the use of ordinary
// function as Expertise mutator.
type ExpertiseFunc func(context.Context, *ent.ExpertiseMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ExpertiseFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ExpertiseMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ExpertiseMutation", m)
	}
	return f(ctx, mv)
}
